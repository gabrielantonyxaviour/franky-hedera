...
start_api_server() {
  log_status "Starting the Franky Agent initialization server..."
  
  # Ensure we clear any previous log files
  ...
  if ! command -v tsx &> /dev/null; then
    log_status "Installing tsx for TypeScript execution..."
    npm install --global tsx
  fi
  
  # Start the initialization server (server.ts) on port 8080
  log_status "Launching initialization server (server.ts)..."
  npx tsx routes/server.ts > "$API_LOG_FILE" 2>&1 &
  API_PID=$!
  
  # Wait a moment for startup
  sleep 2
  
  # Check if initialization server process is running
  if ! ps -p $API_PID > /dev/null; then
    log_error "Initialization server process failed to start. Check $API_LOG_FILE for details."
    cat "$API_LOG_FILE"
    exit 1
  fi
  
  # Wait for initialization server to be ready
  retry_count=0
  max_retries=15
  while [ $retry_count -lt $max_retries ]; do
    if curl -s http://localhost:8080/health 2>/dev/null; then
      log_success "Initialization server is up and running on port 8080!"
      break
    fi
    retry_count=$((retry_count + 1))
    if [ $retry_count -lt $max_retries ]; then
      log_status "Waiting for initialization server to start... ($retry_count/$max_retries)"
      sleep 2
    else
      log_warning "Initialization server status check failed but may still be starting up."
    fi
  done
  
  # Start displaying logs in real-time
  log_status "Server started. Displaying logs:"
  echo "-----------------------------------------"
  ...
  echo "Initialization Server (8080):"
  ...
  echo "-----------------------------------------"
  echo "🌐 Init Server URL: http://localhost:8080"
  echo "🌐 Public URL:      $NGROK_URL"
  echo "-----------------------------------------"
  
  log_success "Franky Agent initialization server is now running! Press Ctrl+C to stop."
